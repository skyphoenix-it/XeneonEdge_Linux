...
> media, time, data, and everyday information. A companion desktop Manager can
> add, resize, reorder, and restyle widgets while the running Hub follows live.
>
> The release includes 30 first-party widgets and 19 preset screens. It stores
...
> Edit from the panel or from your main display. The Manager pushes layout and
> appearance changes to the running Hub over a local socket, including screen
> selection, resizing, reorder, and orientation.
...
>
> The core is Rust, the UI is Qt 6/QML, and configuration is local TOML. There is
> no account requirement or telemetry implementation. Network-capable widgets
> and the opt-in update check use one central egress gate.
>
...
> Show HN: EdgeHub – a Rust + Qt dashboard for the Corsair Xeneon Edge on Linux

**Body:**

> EdgeHub turns the 2560×720 touch display into native, swipeable widget pages.
...
>
> I spent most of the release cycle on the awkward parts: physical display
> targeting, portrait/landscape behavior, reconnects, touch safety, Manager/Hub
...
`#Linux` `#KDE` `#Wayland` `#RustLang` `#Qt` `#QML` `#OpenSource`
